...
{
   LPDIRECTINPUT pdi = (struct IDirectInputA *)pvRef;
   LPDIRECTINPUTDEVICE pdev;
   DIPROPRANGE diprg;
...
DWORD ReadKeyboardInput(void)
{
   ...
   if(hRes != DI_OK)
   {
      ...
      for(dw = 0; dw < dwEvents; dw++)
      {
         ...
         case DIK_SPACE:
            if(rgKeyData[dw].dwData & 0x80)
               dwKeyState |= KEY_ENTER_OR_EXIT_CAR;
            else
               dwKeyState &= (DWORD)~KEY_ENTER_OR_EXIT_CAR;
            break;
	
		case DIK_F12:
            if(rgKeyData[dw].dwData & 0x80)
               dwKeyState |= KEY_SCREENSHOT;
            else
               dwKeyState &= (DWORD)~KEY_SCREENSHOT;
            break;

		case DIK_NUMPADPLUS:
            if(rgKeyData[dw].dwData & 0x80)
               dwKeyState |= KEY_CHANGE_WEAPON_UP;
            else
               dwKeyState &= (DWORD)~KEY_CHANGE_WEAPON_UP;
            break;

         case DIK_NUMPADMINUS:
            if(rgKeyData[dw].dwData & 0x80)
               dwKeyState |= KEY_CHANGE_WEAPON_DOWN;
            else
               dwKeyState &= (DWORD)~KEY_CHANGE_WEAPON_DOWN;
            break;

         case DIK_NUMPAD7:
            if(rgKeyData[dw].dwData & 0x80)
               dwKeyState |= KEY_CHANGE_GEAR_DOWN;
            else
               dwKeyState &= ~KEY_CHANGE_GEAR_DOWN;
            break;
