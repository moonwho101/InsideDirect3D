...
void DisplayRRStats(HWND hwnd)
{
	...
	hr = d3dapp->lpDD7->QueryInterface(IID_IDirectDraw7, (void **)&lpDD7); 
	if (FAILED(hr))
		return; 
	...
	TextOut(hdc, sx1, sy, "D3D Device Desc :",17);
	TextOut(hdc, sx2, sy, D3Ddevicedesc, strlen(D3Ddevicedesc));
	sy +=20;
	...
	if(d3dapp->ThisTextureFormat.bAlphaPixels == TRUE)
		strcat(buffer,"A");
	...
	itoa(setupinfo->TextureSize , buf3,  10);
	strcpy(buffer, buf3);
	strcat(buffer, "x ");
	strcat(buffer, buf3);

	strcpy(buf2, "Texture size :");
			
	TextOut(hdc, sx1, sy, buf2,   strlen(buf2));
	TextOut(hdc, sx2, sy, buffer, strlen(buffer));
	sy +=40;

	itoa(world.num_triangles_in_scene , buffer,10);
	strcpy(buf2, "Number of triangles in scene :");
	TextOut(hdc, sx1, sy, buf2,  strlen(buf2));
	TextOut(hdc, sx2, sy, buffer,strlen(buffer));
	sy +=20;
			
	itoa(world.num_verts_in_scene, buffer,10);
	strcpy(buf2, "Number of verts in scene  :");
	TextOut(hdc, sx1, sy, buf2,   strlen(buf2));
	TextOut(hdc, sx2, sy, buffer, strlen(buffer));
	sy +=20;

	itoa(world.num_dp_commands_in_scene, buffer,10);
