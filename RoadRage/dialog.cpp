...
extern int server_assigned_player_number;
extern WORLD world;
					...
					LPDPLAYINFO lpDPInfo)
{
	...
	sessionDesc.dwFlags = DPSESSION_CLIENTSERVER | DPSESSION_MULTICASTSERVER ; //DPSESSION_MIGRATEHOST | DPSESSION_KEEPALIVE;
	...
	hr = lpDirectPlay4A->CreatePlayer(&dpidPlayer, &dpName, 
							lpDPInfo->hPlayerEvent, NULL, 0, DPPLAYER_SERVERPLAYER);
	if FAILED(hr)
	...
	am_i_the_server = TRUE;
	server_assigned_player_number = 0;
	world.player_list[0].RRnetID = lpDPInfo->dpidPlayer;
	world.player_list[0].dpid = lpDPInfo->dpidPlayer;
	MyRRnetID = lpDPInfo->dpidPlayer;
	world.num_players = 1;

	fp = fopen("rrlogfile.txt","a");
	fprintf( fp, "\nHostSession---------------\n");
	fprintf( fp, "Player name : %s\n",lpszPlayerName);	
	fprintf( fp, "dpidPlayer  : %d\n",dpidPlayer);
					...
					LPDPLAYINFO lpDPInfo)
{
	...
	// create a player with this name
	hr = lpDirectPlay4A->CreatePlayer(&dpidPlayer, &dpName, 
							lpDPInfo->hPlayerEvent, &MyRRnetID, sizeof(MyRRnetID), 0);
	if FAILED(hr)
		goto CREATEPLAYER_FAILURE;
