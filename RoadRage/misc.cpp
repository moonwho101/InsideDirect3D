...
{
    int n;
    char backup[D3DAPP_MAXTEXTURES][D3DAPP_MAX_TEXTURE_FILENAME_LENGTH];

    n = d3dappi.NumTextures;
    memcpy(&backup[0][0], &d3dappi.ImageFile[0][0], D3DAPP_MAX_TEXTURE_FILENAME_LENGTH * D3DAPP_MAXTEXTURES);
    ZEROMEM(d3dappi);
    memcpy(&d3dappi.ImageFile[0][0], &backup[0][0], D3DAPP_MAX_TEXTURE_FILENAME_LENGTH * D3DAPP_MAXTEXTURES);
    ...
    d3dapprs.FogColor = RGB_MAKE(0, 0, 28);
    d3dapprs.FogMode = D3DFOG_LINEAR;
    d3dapprs.FogStart = D3DVAL(200.0);
    d3dapprs.FogEnd = D3DVAL(500.0);
...
BOOL D3DAppICallDeviceCreateCallback(int w, int h)
{
	...
	{
		++CallbackRefCount;
		return (D3DDeviceCreateCallback)(w, h,
...
void D3DAppISetErrorString( LPSTR fmt, ... )
{
	...
	PrintMessage(myglobs.hWndMain, "D3D Error :\n", NULL, LOGFILE_ONLY);
	PrintMessage(myglobs.hWndMain, buff, "\n", LOGFILE_ONLY);
