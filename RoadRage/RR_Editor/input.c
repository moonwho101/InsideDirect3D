   ...
   if(hRes != DI_OK)
   {
	...
	for(dw = 0; dw < dwEvents; dw++)
	{
		switch(rgKeyData[dw].dwOfs)
        {
			...
			case DIK_NUMPADPLUS:
				if(rgKeyData[dw].dwData & 0x80)
					dwKeyState |= KEY_MOVE_UP;
				else
					dwKeyState &= (DWORD)~KEY_MOVE_UP;
				break;

			case DIK_NUMPADMINUS:
				if(rgKeyData[dw].dwData & 0x80)
					dwKeyState |= KEY_MOVE_DOWN;
				else
					dwKeyState &= (DWORD)~KEY_MOVE_DOWN;
				break;

			case DIK_UP:
			case DIK_NUMPAD8:
				if(rgKeyData[dw].dwData & 0x80)
					dwKeyState |= KEY_MOVE_FORWARD;
				else
					dwKeyState &= ~KEY_MOVE_FORWARD;
				break;
