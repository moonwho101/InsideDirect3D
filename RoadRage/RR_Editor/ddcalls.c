				      ...
				      LPSTR lpDriverName, LPVOID lpContext)
{
    ...
    strcpy(buffer, lpDriverDesc);
	strcat(buffer, "  ");
	strcat(buffer, lpDriverName);

	PrintMessage(NULL, "D3DAppIDDEnumCallback: ", buffer, LOGFILE_ONLY);
	...
	{	
		...
		{
			PrintMessage(NULL, "D3DAppIDDEnumCallback: DirectDrawCreate FAILED", NULL, LOGFILE_ONLY);
			return DDENUMRET_OK;
		}
		...
		if (FAILED(lpDD->lpVtbl->GetCaps(lpDD, &DriverCaps, &HELCaps))) 
		{
			...
			PrintMessage(NULL, "D3DAppIDDEnumCallback: GetCaps FAILED", NULL, LOGFILE_ONLY);
			return DDENUMRET_OK;
		}
		...
		if (DriverCaps.dwCaps & DDCAPS_3D) 
		{
			...
			*(LPDIRECTDRAW*)lpContext = lpDD;
			PrintMessage(NULL, "D3DAppIDDEnumCallback: Suceeded\n", NULL, LOGFILE_ONLY);
...
BOOL D3DAppICreateDD(DWORD flags)
{
	...
	PrintMessage(NULL, "D3DAppICreateDD: starting", NULL, LOGFILE_ONLY);
    ...
    if (!(flags & D3DAPP_ONLYDDEMULATION)) 
	{
		...
		if (LastError != DD_OK) 
		{
			...
			PrintMessage(NULL, "D3DAppICreateDD: DirectDrawEnumerate hardware ok", NULL, LOGFILE_ONLY);
	...
	LastError = lpDD->lpVtbl->QueryInterface(lpDD, &IID_IDirectDraw, 
			(LPVOID *)&d3dappi.lpDD);

	if (LastError != DD_OK) 
	{
		lpDD->lpVtbl->Release(lpDD);
		D3DAppISetErrorString("D3DAppICreateDD: Couldn't query for DirectDraw4.\n%s",
			  D3DAppErrorToString(LastError));
		return FALSE;
	}
	...
	PrintMessage(NULL, "D3DAppICreateDD: suceeded", NULL, LOGFILE_ONLY);
    return TRUE;
}
