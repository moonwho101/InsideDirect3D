 ...
 */
LPDIRECTDRAWSURFACE D3DAppILoadSurface(LPDIRECTDRAW2 lpDD, LPCSTR lpName,
...
int CheckValidTextureAlias(HWND hwnd, char *alias, world_ptr wptr)
{
	...
	for(i = 0; i < wptr->number_of_tex_aliases; i++)
	{
...
BOOL LoadTextures(HWND hwnd,BOOL mode , world_ptr wptr)
{
    ...
    fp = fopen("textures.dat","r");
    if(fp==NULL)
    {     
        MessageBox(hwnd,"Error Opening texalias.dat",NULL,MB_OK);
		return FALSE;
    }
