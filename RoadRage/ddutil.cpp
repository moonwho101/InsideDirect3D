...
IDirectDrawSurface7 * DDLoadBitmap(IDirectDraw7 *pdd, LPCSTR szBitmap, int dx, int dy)
{
	...
	//memcpy(&ddsd.ddpfPixelFormat, &d3dappi.ThisTextureFormat.ddsd, sizeof(DDPIXELFORMAT));

	//memcpy(&ddsd.ddpfPixelFormat, &d3dappi.TextureFormat[d3dappi.CurrTextureFormat], sizeof(DDPIXELFORMAT));

	memcpy(&ddsd.ddpfPixelFormat, &d3dappi.ddpfTextureFormats, sizeof(DDPIXELFORMAT));

 
    LastError = d3dappi.lpDD7->CreateSurface(&ddsd, &pdds, NULL);
		
	if(LastError != DD_OK)
	{
		D3DAppISetErrorString("DDLoadBitmap failed : CreateSurface\n%s", D3DAppErrorToString(LastError));
		return NULL;
	}
