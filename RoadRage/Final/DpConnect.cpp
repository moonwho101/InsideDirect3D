    ...
    while( TRUE )
    {
        switch( nStep )
        {
             case 0:
                ...
                nExitCode = DialogBox( hInst, MAKEINTRESOURCE(IDD_MULTIPLAYER_CONNECT), 
                                       g_pD3DApp->Get_hWnd(), DPConnect_ConnectionsDlgProc );
                
				pCMyApp->UpdatePosTimer = timeSetEvent(
							pCMyApp->tx_rate_ps, 0,	
            ...
            case 1:
				...
				g_pD3DApp->Pause(TRUE);
                nExitCode = DialogBox( hInst, MAKEINTRESOURCE(IDD_MULTIPLAYER_GAMES), 
                                       g_pD3DApp->Get_hWnd(), DPConnect_SessionsDlgProc );
...
HRESULT DPConnect_SessionsDlgJoinGame( HWND hDlg )
{
	...
	pCMyApp->MyRRnetID = g_LocalPlayerDPID;
	itoa((int)pCMyApp->MyRRnetID, buffer, 10);
	PrintMessage(NULL, "Direct Play : Joined game", NULL, NULL);
	PrintMessage(NULL, "Player's dpid : ", dpname.lpszShortNameA, NULL);
	PrintMessage(NULL, "MyRRnetID     : ", buffer, NULL);

	pCMyApp->multiplay_flag = TRUE;
...
HRESULT DPConnect_SessionsDlgCreateGame( HWND hDlg )
{
	...
	pCMyApp->player_list[pCMyApp->num_players].x = 600;
	pCMyApp->player_list[pCMyApp->num_players].y = 12;
	pCMyApp->player_list[pCMyApp->num_players].z = 500;
	pCMyApp->player_list[pCMyApp->num_players].rot_angle = 0;
	pCMyApp->player_list[pCMyApp->num_players].model_id = 0;
	pCMyApp->player_list[pCMyApp->num_players].skin_tex_id = 27;
	pCMyApp->player_list[pCMyApp->num_players].current_weapon = 0;
	pCMyApp->player_list[pCMyApp->num_players].current_car = 0;
	pCMyApp->player_list[pCMyApp->num_players].current_frame = 0;
	pCMyApp->player_list[pCMyApp->num_players].current_sequence = 0;
	pCMyApp->player_list[pCMyApp->num_players].bIsPlayerInWalkMode = TRUE;
	pCMyApp->player_list[pCMyApp->num_players].RRnetID = g_LocalPlayerDPID;
	pCMyApp->player_list[pCMyApp->num_players].bIsPlayerValid = TRUE;
	pCMyApp->num_players++;

	
	if(dpname.lpszShortNameA == NULL)
		strcpy(pCMyApp->player_list[0].name, "no name");
	else
		strcpy(pCMyApp->player_list[0].name, dpname.lpszShortNameA);

	fp = fopen("rrlogfile.txt","a");
	fprintf( fp, "Direct Play : Created game\n");
	fprintf( fp, "Player's name  : %s\n", pCMyApp->player_list[0].name);
	fprintf( fp, "Player's dpid  : %d\n", pCMyApp->player_list[0].RRnetID);
	fprintf( fp, "MyRRnetID      : %d\n", pCMyApp->MyRRnetID);
	fprintf( fp, "---------------\n\n");
	fclose(fp);
